...
class SSD_512(object):
    ...
    def build(self, feed_vars, mode='train'):
        ...
        #     del(new_feats[1])
        #     print(new_feats)
        # body_feats = OrderedDict([('new_feats{}concat'.format(idx),feat)
        ...
        if isinstance(body_feats, OrderedDict):
            ...
            new_feats = body_feats
            # print(body_feats)
            new_feats_change = self.BasicConv(body_feats[0], 256, filter_size = 1, down_size = 64)
            new_feats_change2 = self.BasicConv(body_feats[1], 256, filter_size = 1)
            # print(new_feats_change, new_feats_change2)
            new_feats[0] = fluid.layers.concat([new_feats_change, new_feats_change2], axis = 1)
            # print(3)
        ...
        body_feats2 = []
        body_feats3 = []
        num_of_layers = [512,256,256,256,256,256,256]
        i = 0
        for body_feat in body_feats:
            ...
            body_feats2 += [self._squeeze_excitation1(body_feat,num_of_layers[i],name = name2)]
        ...
        locs, confs, box, box_var = self.multi_box_head(
            inputs=body_feats3, image=im, num_classes=self.num_classes)
