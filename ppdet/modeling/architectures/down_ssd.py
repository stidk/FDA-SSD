...
class SSD(object):
    ...
    def build(self, feed_vars, mode='train'):
        ...
        for i in range(len(body_feats1)):
            body_feats11 += [self.BasicConv(body_feats1[i], layers[i], filter_size = 1)]
        body_feats1 = body_feats11
        ...
        for body_feat in body_feats1:
            ...
            body_feats_w1 += [self._squeeze_excitation1(body_feat,num_of_layers[i],name = name2)]
            i += 1
        body_feast_all_w = fluid.layers.concat([body_feats_w[0], body_feats_w[1], body_feats_w[2], body_feats_w[3]], axis=1)
        body_feast_all_w1 = fluid.layers.concat([body_feats_w1[0], body_feats_w1[1], body_feats_w1[2], body_feats_w1[3]], axis=1)
        ...
        for i in range(len(num_of_layers)):
            ...
            body_feats_w3 = fluid.layers.fc(
            input=body_feast_all_w1,
            size=layers[i],
            act='sigmoid')
            body_feats3 += [fluid.layers.elementwise_mul(x=body_feats1[i], y=body_feats_w3, axis=0)]
        for i in range(4):
            # print(body_feats2[3])
            body_feats4 += [fluid.layers.concat([body_feats2[i], body_feats3[i]], axis=1)]
        ...
        body_feats2 = OrderedDict([('new_feats{}'.format(idx),feat)
                                for idx, feat in enumerate(body_feats4)])
